--[[ This file was getting created by @phnglcth7 ]]

loadstring(game:HttpGet("https://raw.githubusercontent.com/Seven7-lua/RedzLibs/refs/heads/main/src/RedzlibV2/source.lua"))()

MakeWindow({
    Hub = { Title = "Linh tinh Hub : Blox fruit", 
   Animation = "Tạo bởi @phnglcth7" },
    Key = {
        KeySystem = false,
        Title = "Key System",
        Description = "key ko cần vượt",
        KeyLink = "https://pastefy.app/7kwbzZSn/raw",
        Keys = {"Top-67-script"},
        Notifi = {
            Notifications = true,
...
-- PlaceId của Blox Fruits
local Sea1 = 2753915549
local Sea2 = 4442272183
local Sea3 = 7449423635

-- Check PlaceId
if PlaceId ~= Sea1 and PlaceId ~= Sea2 and PlaceId ~= Sea3 then
    player:Kick("Game này không được hỗ trợ!")
end
local replicated = game:GetService("ReplicatedStorage")

function SetTeamPirates()
    replicated.Remotes.CommF_:InvokeServer("SetTeam", "Pirates")
end
function SetTeamMarines()
        ...
        pcall(function()
            if IslandESP then
                if island.Name ~= "Sea" then
                    local gui = island:FindFirstChild("NameEsp")
                    if not gui then
                        ...
                        label.TextColor3 = Color3.fromRGB(8, 0, 0)
                        label.Parent = gui
                    end

                    local distance = roundNumber((head.Position - island.Position).Magnitude / 3)
                    gui:FindFirstChild("Label").Text = island.Name .. "\n" .. distance .. " Distance"
                end
            else
                local gui = island:FindFirstChild("NameEsp")
    ...
    for _, target in pairs(game:GetService("Players"):GetChildren()) do
        pcall(function()
            if not isnil(target.Character) then
                ...
                if ESPPlayer and head and humanoid then
                    ...
                    if not gui then
                        ...
                        label.BackgroundTransparency = 1
                        label.TextStrokeTransparency = 0.5
                        label.TextColor3 = (target.Team == player.Team) and Color3.new(0,0,254) or Color3.new(1,0,0)
                        label.Parent = gui
                    end

                    local distance = roundNumber((player.Character.Head.Position - head.Position).Magnitude / 3)
                    local healthPercent = roundNumber((humanoid.Health * 100) / humanoid.MaxHealth)
                    gui:FindFirstChild("Label").Text = target.Name .. " | " .. distance .. " Distance\nHealth: " .. healthPercent .. "%"
    ...
    for _, obj in pairs(game.Workspace:GetChildren()) do
        pcall(function()
            if string.find(obj.Name, "Chest") then
                if ChestESP then
                    local gui = obj:FindFirstChild("NameEsp" .. Number)
                    if not gui then
                        gui = Instance.new("BillboardGui")
                        gui.Name = "NameEsp" .. Number
                        gui.ExtentsOffset = Vector3.new(0, 1, 0)
                        ...
                        -- Gán màu và text theo chest
                        local distance = v20((head.Position - obj.Position).Magnitude / 3)
                        ...
                        -- Update khoảng cách nếu đã có gui
                        local distance = v20((head.Position - obj.Position).Magnitude / 3)
                        gui:FindFirstChild("Label").Text = obj.Name .. "\n" .. distance .. " Distance"
                    end
                else
                    -- Xóa gui nếu ChestESP tắt
                    local gui = obj:FindFirstChild("NameEsp" .. Number)
                    if gui then gui:Destroy() end
                end
            end
        ...
        pcall(function()
            if DevilFruitESP then
                if string.find(obj.Name, "Fruit") and obj:FindFirstChild("Handle") then
                    ...
                    local gui = handle:FindFirstChild("NameEsp" .. Number)
                    if not gui then
                        gui = Instance.new("BillboardGui")
                        gui.Name = "NameEsp" .. Number
                        gui.ExtentsOffset = Vector3.new(0, 1, 0)
                        ...
                        local distance = v20((head.Position - handle.Position).Magnitude / 3)
                        label.Text = obj.Name .. "\n" .. distance .. " Distance"
                    else
                        local distance = v20((head.Position - handle.Position).Magnitude / 3)
                        gui:FindFirstChild("Label").Text = obj.Name .. "\n" .. distance .. " Distance"
...
function AutoEquipMelee()
    ...
    local function equip()
        ...
        -- Nếu đang cầm melee thì thôi
        local tool = char:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("ToolTip") and tool.ToolTip == "Melee" then
            return
        end

        -- Tìm melee trong backpack
        for _, t in pairs(backpack:GetChildren()) do
            if t:IsA("Tool") and t:FindFirstChild("ToolTip") and t.ToolTip == "Melee" then
                t.Parent = char
                return
            end
        end
...
function AutoEquipFruit()
    ...
    local function equipFruit()
        ...
        -- Nếu đang cầm trái thì bỏ qua
        local tool = char:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("ToolTip") and tool.ToolTip == "Blox Fruit" then
            return
        end

        -- Tìm tool trái trong Backpack
        for _, t in pairs(backpack:GetChildren()) do
            if t:IsA("Tool") and t:FindFirstChild("ToolTip") and t.ToolTip == "Blox Fruit" then
                t.Parent = char
                return
            end
        end
...
function AutoEquipSword()
    ...
    local function equipSword()
        ...
        -- Nếu đang cầm sword thì thôi
        local tool = char:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("ToolTip") and tool.ToolTip == "Sword" then
            return
        end

        -- Tìm sword trong Backpack
        for _, t in pairs(backpack:GetChildren()) do
            if t:IsA("Tool") and t:FindFirstChild("ToolTip") and t.ToolTip == "Sword" then
                t.Parent = char
                return
            end
        end
        ...
        while task.wait(0.05) do
            if _G.AutoGun then
                ...
                if char then
                    ...
                    -- Nếu đang cầm Gun → lưu lại
                    if tool and tool.ToolTip == "Gun" then
                        currentGun = tool
                    end

                    -- Nếu đổi tool → equip lại Gun
                    if currentGun and (not tool or tool ~= currentGun) then
                        local hum = char:FindFirstChildOfClass("Humanoid")
                        if hum then
                            hum:EquipTool(currentGun)
                        end
                    end
                end
            end
...
function TpFruit()
    for _, v in pairs(workspace:GetDescendants()) do
        ...
        if string.find(v.Name:lower(), "fruit") then
            ...
            -- Nếu là Model (lấy PrimaryPart)
            if v:IsA("Model") and v.PrimaryPart then
                _tp(v.PrimaryPart.CFrame)
                return true
            end

            -- Nếu là Model nhưng chưa set PrimaryPart
            if v:IsA("Model") then
...
function FixLag()
    task.spawn(function()
        while task.wait(3) do
            pcall(function()
                if not _G.ReduceLag then return end
...
function CheckFruit()
    if GetBP("Dragon-Dragon") or GetBP("Gas-Gas") or GetBP("Yeti-Yeti") or GetBP("Kitsune-Kitsune") or GetBP("T-Rex-T-Rex") or GetBP("Rocket-Rocket") then
        return true
    end
end
