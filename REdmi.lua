-- Blox Fruit Ultimate Script - Full Features A-Z
-- Complete Auto Farm System với CFrame
...
-- Hàm Teleport sử dụng CFrame
function TeleportToCFrame(targetCFrame)
    pcall(function()
...
-- Hàm Tween Teleport mượt mà
function TweenTeleport(targetCFrame)
    pcall(function()
        local character = Player.Character
        if character and character:FindFirstChild("HumanoidRootPart") then
            local tweenInfo = TweenInfo.new(
                (character.HumanoidRootPart.Position - targetCFrame.Position).Magnitude / Config.TeleportSpeed,
...
function FindNearestMob()
    ...
    for _, mob in pairs(Workspace.Enemies:GetChildren()) do
        if mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob.Name == NameMon then
    ...
    spawn(function()
        while Config.AutoSell do
            pcall(function()
                ...
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Cousin", "Deliver")
            end)
            wait(60) -- Chờ 1 phút
        end
    end)
        ...
        while Config.AutoCollectDrops do
            pcall(function()
                for _, drop in pairs(Workspace:GetChildren()) do
                    if drop.Name == "Fruit" or drop.Name == "Microchip" then
                        TweenTeleport(drop.CFrame)
                        wait(0.5)
                    end
                end
            end)
...
function AutoBossFarm()
    spawn(function()
        while Config.AutoBoss do
            pcall(function()
                -- Thêm code farm boss ở đây
                local bosses = {"Darkbeard", "Swan", "Don Swan", "Cursed Captain"}
                for _, bossName in pairs(bosses) do
                    local boss = Workspace:FindFirstChild(bossName)
                    if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 then
                        TweenTeleport(boss.HumanoidRootPart.CFrame)
                        AttackTarget(boss)
                        break
                    end
                end
...
-- Auto Stats
function AutoStatsUp()
    spawn(function()
        while Config.AutoStats do
            pcall(function()
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AddPoint", Config.SelectedStats, 1)
            end)
            wait(0.5)
        end
    end)
...
-- GUI Creation với Orion Lib
local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({
    Name = "🔥 Blox Fruit Ultimate Script", 
    HidePremium = false, 
    SaveConfig = true, 
...
MainTab:AddDropdown({
    Name = "🎯 Select Mob",
    Default = "Nearest",
    Options = {"Nearest", "Bandit", "Monkey", "Pirate", "Brute", "Snow Bandit", "Desert Bandit"},
    Callback = function(Value)
        Config.SelectMonster = Value
    end    
})
...
TeleportTab:AddDropdown({
    ...
    Options = {"Starter Island", "Jungle", "Pirate Village", "Desert", "Snow Mountain", "Marine Fortress"},
    Callback = function(Value)
        -- Thêm CFrame cho các đảo
        local IslandCFrames = {
            ["Starter Island"] = CFrame.new(100, 15, 1300),
            ["Jungle"] = CFrame.new(-1600, 37, 150),
            -- Thêm các CFrame khác...
        }
        if IslandCFrames[Value] then
            TeleportToCFrame(IslandCFrames[Value])
        end
    end    
})
...
-- Khởi tạo
OrionLib:Init()
print("🔥 Blox Fruit Ultimate Script Loaded Successfully!")
print("📁 Config saved in: BloxFruitConfig")
